...
const ALL_TYPES: readonly PostCardImageType[] = [1, 2, 3, 4, 5, 6];
...
describe("PostCardImage", () => {
  it("고정 크기(160x102)의 img로 렌더링된다", () => {
    const container = renderIntoDocument(
      <PostCardImage type={1} alt="썸네일" />,
    );
  ...
  it("전달한 alt가 그대로 반영된다 (필수 prop)", () => {
    const container = renderIntoDocument(
      <PostCardImage type={2} alt="게시글 썸네일 설명" />,
    );
    const img = getByRole(container, "img");

    expect(img.getAttribute("alt")).toBe("게시글 썸네일 설명");
  });

  it("전달한 className이 기본 클래스에 추가로 반영된다", () => {
    const container = renderIntoDocument(
      <PostCardImage type={1} alt="썸네일" className="custom-class" />,
    );
    const img = getByRole(container, "img");
  ...
  it("토큰 기반 라운드/보더 클래스를 사용한다 (하드코딩 색상/spacing 금지)", () => {
    ...
    expect(img.className).toContain("rounded-[var(--radius-sm)]");
  ...
  it("type=1~6 각각 다른 이미지 src를 렌더링한다", () => {
    const srcs = ALL_TYPES.map((type) => {
      const container = renderIntoDocument(
        <PostCardImage type={type} alt={`썸네일 ${type}`} />,
      );
      const img = getByRole(container, "img");
  ...
  it("data-post-card-image 속성에 현재 type을 노출한다", () => {
    const container = renderIntoDocument(
      <PostCardImage type={4} alt="썸네일" />,
    );
    const img = getByRole(container, "img");

    expect(img.getAttribute("data-post-card-image")).toBe("4");
  });
});
